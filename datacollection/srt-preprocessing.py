...
def process_srt_file(srt_file, skip_lines=15, max_time_gap=3.0):
    ...
    # Rremove first and last lines to avoid unnecessary information (e.g. about author)
    ...
    # remove lines that are not conversation lines or time frames once again
    lines = [line for line in lines if not re.match(r'^\d+\s*$', line)]
...
if __name__ == "__main__":
    ...
    final_file = "datasets/srt/srt-conversations-new.txt"
    ...
    for filename in os.listdir(folder_path):
        if filename.endswith(".srt"):
            ...
            if conversations is None:
                print(f"Usuwanie problematycznego pliku: {srt_file_path}")
